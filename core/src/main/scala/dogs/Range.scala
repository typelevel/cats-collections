...
/**
  ...
  */
class Range[A](val start: A, val end: A) {
  /**
  ...
  def withValue[B](value: B): VRange[A, B] = VRange(this, value)

  override def equals(obj: scala.Any): Boolean = obj match {
    case Range(`start`, `end`)  =>  true
    case _                      =>  false
  }
}
...
object Range {
  ...
  def apply[A](start: A, end: A): Range[A] = new Range(start, end)

  def unapply[A](arg: Range[A]): scala.Option[(A, A)] = scala.Some((arg.start, arg.end))
...
class VRange[A, B] private [dogs] (override val start: A, override val end: A, val value: B) extends Range[A](start, end) {
  override def equals(obj: Any): Boolean = obj match {
    case VRange(`start`, `end`, `value`)  =>  true
    case _                                =>  false
  }

  override def toString(): String = s"VRange($start, $end, $value)"
}

object VRange {
  def apply[A, B](start: A, end: A, value: B): VRange[A, B] = new VRange(start, end, value)

  def apply[A, B](range: Range[A], value: B): VRange[A, B] = new VRange(range.start, range.end, value)

  def unapply[A, B](vRange: VRange[A, B]): scala.Option[(A, A, B)] = scala.Some((vRange.start, vRange.end, vRange.value))
}